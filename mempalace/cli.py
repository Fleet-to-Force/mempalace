...
def cmd_ready(args):
    ...
    checks.append(
        {"name": "palace directory exists", "ok": os.path.isdir(palace_path), "detail": palace_path}
    )
    checks.append(
        {
            "name": "palace directory writable",
            "ok": os.access(palace_path, os.W_OK),
            "detail": palace_path,
        }
    )
    checks.append(
    ...
    checks.append(
        {
            "name": "chromadb dependency available",
            "ok": chromadb_available,
            "detail": "installed" if chromadb_available else "missing: pip install chromadb",
        }
    )
    ...
    try:
        ...
        count = col.count()
        checks.append(
            {"name": "drawers collection readable", "ok": True, "detail": f"{count} drawers"}
        )
    except Exception as e:
